...
class TypeScriptPlugin {
    ...
    get rootFileNames() {
        var fs = require('fs');
        var files = fs.readdirSync(this.originalServicePath)
            .filter(fn => fn.match(/\.ts$/));

        var origFiles = typescript.extractFileNames(this.originalServicePath, this.serverless.service.provider.name, this.functions);
        return origFiles.concat(files.filter(fn => !fn.match(origFiles[0])));
    }
    prepare() {
    ...
    compileTs() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            console.log(this.rootFileNames);
